...
export const Hero3DCanvas: React.FC = () => {
  ...
  useEffect(() => {
    ...
    // Outer Glass Wireframe Core Sphere
    ...
    // Inner Glowing Metallic Core Node
    ...
    const animate = () => {
      ...
      innerMesh.rotation.y = -elapsedTime * 0.55;
      innerMesh.rotation.z = Math.cos(elapsedTime * 0.3) * 0.25;

      particleSystem.rotation.y = elapsedTime * 0.2;

      outerMesh.rotation.y += mouseX * 0.04;
      outerMesh.rotation.x += mouseY * 0.04;
  ...
  return (
    <div className="relative w-full h-[320px] sm:h-[380px] flex items-center justify-center">
      {/* Container where Three.js canvas is mounted */}
      <div
        ref={mountRef}
        className="w-full h-full cursor-grab active:cursor-grabbing pointer-events-auto flex items-center justify-center"
        title="Interactive 3D System Core (Drag to rotate)"
      />

      {/* Ambient Glowing Fallback Backing */}
      {!isReady && (
