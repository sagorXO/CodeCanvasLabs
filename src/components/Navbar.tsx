...
import { Layers, Sparkles, Terminal, Shield, ArrowRight, Menu, X, Code, HelpCircle, Activity, Search, Calculator, Calendar } from 'lucide-react';
...
export const Navbar: React.FC<NavbarProps> = ({ onOpenConsultation, onOpenWaitlist, onOpenLookup }) => {
      ...
      <div className="mx-auto flex max-w-7xl items-center justify-between px-6">
        ...
        <a href="#" className="flex items-center gap-3 group">
          ...
          <span className="text-base font-bold tracking-tight text-white flex items-center gap-2">
            Saied Sagar <span className="text-[10px] font-mono font-semibold px-2 py-0.5 rounded-full bg-cyan-500/15 text-cyan-300 border border-cyan-500/30">Studio</span>
          </span>
        </a>

        {/* Navigation Links */}
        <div className="flex items-center gap-6">
          <nav className="hidden xl:flex items-center gap-6 text-xs font-mono tracking-wide text-slate-300/80">
            <a href="#cases" className="hover:text-cyan-300 transition-colors flex items-center gap-1.5 py-1">
              <Layers className="h-3.5 w-3.5 text-cyan-400" /> Case Studies
            </a>
            <a href="#calculator" className="hover:text-cyan-300 transition-colors flex items-center gap-1.5 py-1">
              <Calculator className="h-3.5 w-3.5 text-cyan-400" /> Scope Estimator
            </a>
            <a href="#simulator" className="hover:text-cyan-300 transition-colors flex items-center gap-1.5 py-1">
              <Terminal className="h-3.5 w-3.5 text-blue-400" /> AI Simulator
            </a>
            ...
            <a href="#stack" className="hover:text-cyan-300 transition-colors flex items-center gap-1.5 py-1">
              <Code className="h-3.5 w-3.5 text-purple-400" /> Tech Stack
            </a>
          </nav>
      ...
      <AnimatePresence>
        {isMobileMenuOpen && (
          <motion.div
            ...
            className="xl:hidden border-b border-white/[0.08] bg-[#060507]/95 backdrop-blur-2xl px-6 py-6 overflow-hidden"
          >
            <nav className="flex flex-col gap-4 font-mono text-sm text-slate-300">
              ...
              >
                <Layers className="h-4 w-4 text-cyan-400" /> Case Studies
              </a>
              <a
                href="#calculator"
                onClick={() => setIsMobileMenuOpen(false)}
                className="flex items-center gap-2 py-2 hover:text-cyan-400 transition-colors"
              >
                <Calculator className="h-4 w-4 text-cyan-400" /> Scope Estimator
              </a>
              <a
                href="#simulator"
                onClick={() => setIsMobileMenuOpen(false)}
                className="flex items-center gap-2 py-2 hover:text-cyan-400 transition-colors"
              >
                <Terminal className="h-4 w-4 text-blue-400" /> AI Simulator
              </a>
              <a
                href="#benchmark"
                onClick={() => setIsMobileMenuOpen(false)}
              ...
              <a
                ...
                className="flex items-center gap-2 py-2 hover:text-cyan-400 transition-colors"
              >
                <Code className="h-4 w-4 text-purple-400" /> Tech Stack
              </a>
              <button
                onClick={() => {
              ...
              >
                <Calendar className="h-4 w-4" /> Book Discovery Call ($10k+)
              </button>
            </nav>
          </motion.div>
        )}
